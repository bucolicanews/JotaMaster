...
export const ChatInterface = () => {
  ...
  useEffect(() => {
    ...
    // Busca Skills do Banco
    if (session?.user) {
      fetchDbSkills(session.user.id).then(skills => {
  ...
  const filteredSkills = availableSkills.filter(s => s.name.toLowerCase().includes(mentionFilter.toLowerCase()));
  ...
  const handleKeyDown = (e: React.KeyboardEvent) => {
    if (showMentionMenu && filteredSkills.length > 0) {
      if (e.key === 'ArrowDown') { e.preventDefault(); setSelectedIndex(prev => (prev + 1) % filteredSkills.length); } 
      else if (e.key === 'ArrowUp') { e.preventDefault(); setSelectedIndex(prev => (prev - 1 + filteredSkills.length) % filteredSkills.length); } 
      else if (e.key === 'Enter' || e.key === 'Tab') { e.preventDefault(); if (filteredSkills[selectedIndex]) insertSkill(filteredSkills[selectedIndex].name); } 
  ...
  const handleSend = async () => {
    ...
    if (!apiKey) return toast.error("Configure sua Gemini API Key nas Configurações primeiro.");
    ...
    try {
      const responseText = await sendChatMessage(newHistory, apiKey, availableSkills, (toolName) => {
            ...
            {availableSkills.length > 0 && (
              <div className="hidden md:flex items-center gap-1.5">
                ...
                <div className="flex gap-1">
                  {availableSkills.slice(0, 3).map(s => <Badge key={s.id} variant="outline" className="text-[8px] py-0 h-4 bg-emerald-500/5 text-emerald-600 border-emerald-500/20">{s.name}</Badge>)}
            ...
            <div className="space-y-6 max-w-4xl mx-auto pb-4">
              {messages.length === 0 && (
                <div className="flex flex-col items-center justify-center h-64 text-center space-y-4 opacity-50">
                  ...
                  <div>
                    ...
                    <p className="text-[10px] mt-2 text-primary font-medium">Dica: Digite <span className="font-bold">@</span> para listar suas ferramentas.</p>
                  </div>
                </div>
              )}
              ...
              {activeTool && (
                <div className="flex gap-3 animate-pulse">
                  ...
                  <div className="bg-emerald-500/5 border border-emerald-500/20 rounded-2xl px-4 py-2 text-[10px] font-mono text-emerald-600">Usando ferramenta: <span className="font-bold">{activeTool}</span>...</div>
                </div>
              )}
          ...
          <div className="p-4 border-t border-border/50 bg-muted/10 relative overflow-visible">
            {showMentionMenu && filteredSkills.length > 0 && (
              <div className="absolute bottom-full left-4 mb-2 w-72 bg-card border border-border rounded-lg shadow-2xl overflow-hidden z-[100] animate-in slide-in-from-bottom-2">
                <div className="bg-muted/80 px-3 py-2 border-b border-border flex items-center gap-2"><Terminal className="h-3 w-3 text-primary" /><span className="text-[10px] font-bold uppercase text-muted-foreground">Minhas Ferramentas</span></div>
                <div className="max-h-60 overflow-y-auto">
                  {filteredSkills.map((skill, idx) => (
                    <div key={skill.id} className={cn("px-3 py-2 cursor-pointer flex flex-col gap-0.5 transition-colors", idx === selectedIndex ? "bg-primary/10 border-l-4 border-primary" : "hover:bg-muted/30 border-l-4 border-transparent")} onClick={() => insertSkill(skill.name)}>
                      <div className="flex items-center justify-between"><span className="text-xs font-bold text-foreground">{skill.name}</span><Badge variant="outline" className="text-[8px] h-3 px-1 uppercase opacity-50">Skill</Badge></div>
            ...
            <div className="max-w-4xl mx-auto flex items-end gap-2">
              <Textarea ref={inputRef} placeholder="Digite sua dúvida técnica ou use @ para chamar uma Skill..." value={input} onChange={handleInputChange} onKeyDown={handleKeyDown} className="flex-1 bg-background min-h-[44px] max-h-[600px] resize-y py-3 px-4 text-base overflow-y-auto transition-[height] duration-100" disabled={isLoading} autoComplete="off" rows={1} />
