...
export const Layout: React.FC<LayoutProps> = ({ children }) => {
  ...
  useEffect(() => {
    const fetchInstalledModules = async () => {
      ...
      try {
        const { data: installedData } = await supabase.from('installed_modules').select('module_id').eq('user_id', session.user.id).eq('is_active', true);
        if (!installedData || installedData.length === 0) {
          setDynamicModules([]);
          return;
        }
        const moduleIds = installedData.map(item => item.module_id);
        const { data: systemData } = await supabase.from('system_modules').select('id, name, module_type').in('id', moduleIds);
      ...
      } catch (err) { console.error("[Layout] Erro ao carregar módulos:", err); }
    };
    ...
    if (autenticado && session?.user && !isAdmin) {
      const channel = supabase.channel('wallet_changes').on('postgres_changes', { event: 'UPDATE', schema: 'public', table: 'wallets', filter: `user_id=eq.${session.user.id}` }, (payload) => { setBalance(payload.new.balance); }).subscribe();
  ...
  const NavButton = ({ item, isLocked = false, isDynamic = false }: { item: any, isLocked?: boolean, isDynamic?: boolean }) => {
    ...
    return (
      <Link to={item.to} className="block w-full" onClick={() => setIsMobileOpen(false)}>
        <Button variant="ghost" className={cn("w-full justify-start py-2 px-3 h-10 transition-colors text-sm font-medium", isActive ? "bg-primary text-primary-foreground hover:bg-primary/90" : isDynamic ? "text-foreground hover:bg-primary/10 hover:text-primary border-l-2 border-transparent hover:border-primary" : "text-muted-foreground hover:text-foreground hover:bg-muted/50")}>
            ...
            <div className="space-y-1 mt-6">
              ...
              {isAdmin ? (
                <Link to="/admin/finance" className="block w-full" onClick={() => setIsMobileOpen(false)}>
                  <Button variant="ghost" className={cn("w-full justify-start py-2 px-3 h-12 transition-all border border-blue-500/20 bg-blue-500/5 hover:bg-blue-500/10", location.pathname === '/admin/finance' && "bg-blue-500/20 border-blue-500/40")}>
                ...
                <Link to="/credits" className="block w-full" onClick={() => setIsMobileOpen(false)}>
                  <Button variant="ghost" className={cn("w-full justify-start py-2 px-3 h-12 transition-all border border-primary/20 bg-primary/5 hover:bg-primary/10", location.pathname === '/credits' && "bg-primary/20 border-primary/40")}>
                    ...
                    <div className="flex flex-col items-start">
                      ...
                      <span className="text-sm font-black text-primary leading-none">{balance !== null ? `${balance} Créditos` : '...'}</span>
                    </div>
                  </Button>
                </Link>
              )}
      ...
      <div className="p-4 border-t border-border shrink-0 bg-muted/10">
        {autenticado && (
          <div className="flex flex-col gap-3">
            ...
            <Button size="sm" variant="outline" className="w-full text-destructive border-destructive/20" onClick={handleLogout}><LogOut className="h-4 w-4 mr-2" /> Sair</Button>
          </div>
        )}
      </div>
    </div>
      ...
      {!isFullscreenModule && (
        <aside className="hidden lg:flex flex-col w-64 border-r border-border bg-card h-screen shrink-0">
          <div className="p-4 border-b border-border bg-gradient-primary flex items-center gap-3 shrink-0">
            <div className="rounded-lg bg-black/30 p-2 backdrop-blur shrink-0"><img src="/jota-contabilidade-logo.png" alt="Logo" className="h-8 w-8" /></div>
            <div className="overflow-hidden"><h1 className="text-lg font-bold text-black truncate leading-tight">JOTA</h1><p className="text-[10px] text-black/70 truncate leading-tight">Placa-mãe Inteligente</p></div>
          </div>
          <NavigationContent />
        </aside>
      )}
